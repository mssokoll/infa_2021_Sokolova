...
string kkkk(Rectangle d) //переводит числа в строку
{
	stringstream con;
	string h;
	string x;
...
int main()
{
	...
	while (ss.find("*") != string::npos) // выполняем приоритетные операции и склеиваем строки обратно
	{
		...
		ss = ss.substr(0, i) + ds + ss.substr(j + 1, ss.size() - j - 1); //Склеивание строки из того, что получилось с помощью приоритетной операции, и всего остального
	}
